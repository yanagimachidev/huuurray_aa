...
// MessageNiceListActivity
class MessageNiceListActivity : AppCompatActivity(),
    MessageNiceGetLoaderInterface {

    // 変数定義
    private val LOG_TAG = MessageNiceListActivity::class.java.simpleName // ログ用にクラス名を取得
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startMessageNiceGetLoader(id, 0)
    ...
    // startMessageNiceGetLoader
    private fun startMessageNiceGetLoader(id: Int, page: Int) {
        // 引数に値を渡してローダーを起動
        val bundle = Bundle()
        bundle.putInt("id", id)
        bundle.putInt("page", page)
        supportLoaderManager.restartLoader(13316, bundle,
            MessageNiceGetLoaderCallbacks(this, this))
    }

    // MessageNiceGetLoaderOnLoadFinished
    override fun MessageNiceGetLoaderOnLoadFinished(data: List<AccountData>?) {
        if (onPauseFlg) {
            onPauseFlg = false
        } else {
            // ProgressBarの削除
    ...
    private fun newEndlessScrollListener(layoutManager: LinearLayoutManager) : EndlessScrollListener {
        val endlessScrollListener = object : EndlessScrollListener(layoutManager) {
            override fun onLoadMore(currentPage: Int) {
                startMessageNiceGetLoader(id, currentPage)
            }
